...
The residual plots for Model 9 are:
...
Taking the log of the dependent variable gives us a better distribution of the residuals and turns the influential outlier into a noninfluential outlier. It does not completely get rid of the heteroscedasticity but that is not a big problem since we can just use robust standard errors for t-distribution and F-distribution testing if necessary. 
...
Similarly, the VIF values for Model 8 indicate that there is no multicollinearity. 

```{r, results='markup'}
vif(lm9)
```

The VIF values drop in Model 9 to 1.19.
...
The VIF values in Model 10 increse to 3.28 again, but there is still no evidence of multicollinearity within these models. 
