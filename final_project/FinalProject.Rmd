...
```{r,results='markup'}
res <- resid(fullmodel)
plot1 <- plot(fitted.values(fullmodel),res,xlab = "fitted values",ylab = "residuals",main = "residual plot") + abline(0,0)
plot1
```
From the residual plot above, it is clear that there is outlier in the plot. But most of the residuals are the same value.


4. Multicollinearity 

We can use `vif()` to check the multicollinearity of these variables. Accroding to the result, we can make the conclusion that variables `Sex`, `Height`, `Age`, whose vif is less than 5, are mildly correlated, and these are acceptable. Other variables are highly correlated.
